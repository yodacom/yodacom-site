...
<p>CoinRoc's grid strategy operates under the direction of an adaptive fuzzy inference system (Adaptive Grid FIS) that classifies market regimes and determines whether conditions are suitable for grid trading. When the market is strongly trending &mdash; the environment where grid strategies historically lose &mdash; the FIS steps the strategy to a cash-hold state. This is intended design behavior, not a bug.</p>

<p>The problem arose in how the previous grading system evaluated performance: it used a blended full-period return &mdash; total portfolio return across the entire evaluation window, including periods when the FIS had correctly held cash. For an asset that trended down 40% while the FIS was in a cash-hold state, the blended return absorbed that 40% directional move even though the grid strategy had deliberately not participated in it.</p>
